...
from scheduler_config import (
    ...
    build_scheduler_from_trial, create_scheduler, is_step_per_batch,
...
def run(run_config, hparams, seeds, dl_train, dl_val, device='cpu'):
    ...
    for seed in seeds:
        ...
        trainer = Trainer(
            ...
            acc=True, device=device, step_per_batch=step_per_batch
        )
...
def main():
    ...
    parser.add_argument('--project', type=str, default="HyperbolicLR-CIFAR10-New")
    ...
    model_idx = survey.routines.select(
        "Select model",
        options=model_names
    )
    model = MODEL_REGISTRY[model_names[model_idx]]
